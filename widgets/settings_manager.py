...
from PyQt5.QtWidgets import (QCheckBox, QDialog, QDoubleSpinBox, QHBoxLayout,
                             QLabel, QLineEdit, QScrollArea, QSpinBox, QStyle,
...
class SettingWidget(QWidget):
    def __init__(
        self, setting_property: SettingProperty, parent=None
    ):
        super().__init__(parent=parent)
        ...
        self.name_widget = SettingLabel(f"{self.title} > <strong>{self.name}</strong>", self)
    ...
    def setup(self):
        ...
        if self.setting_property.type == 'boolean':
            self.dynamic_widget = QCheckBox()
            self.dynamic_widget.setText(self.description)
